...
def process_one(filename, hps, hmodel, f0p, device, diff=False, mel_extractor=None):
...
def process_batch(hps, file_chunk, f0p, diff=False, mel_extractor=None, device="cpu"):
    ...
    hmodel = utils.get_speech_encoder(hps["model"]["speech_encoder"], device=device)
    print(f"Loaded speech encoder for rank {rank}")
    for filename in tqdm(file_chunk, position = rank):
        process_one(filename, hps, hmodel, f0p, device, diff, mel_extractor)

def parallel_process(filenames, num_processes, hps, f0p, diff, mel_extractor, device):
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        for i in range(num_processes):
            ...
            tasks.append(executor.submit(process_batch, hps, file_chunk, f0p, diff, mel_extractor, device=device))
...
def main(args):
    ...
    print("Using SpeechEncoder: " + speech_encoder)
    print("Using extractor: " + f0p)
    print("Using diff Mode: " + str(args.use_diff))
    ...
    parallel_process(filenames, num_processes, hps, f0p, args.use_diff, mel_extractor, device)
